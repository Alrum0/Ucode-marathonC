...
void mx_print_variations(char *operand1, char *operation, char *operand2,
                         char *result) {
  if (*operation == '?') {
    mx_print_variations(operand1, "+", operand2, result);
    mx_print_variations(operand1, "-", operand2, result);
    mx_print_variations(operand1, "*", operand2, result);
    mx_print_variations(operand1, "/", operand2, result);
    return;
  ...
  for (long long i = operand1_int; i < mx_pow(10, operand1_len); i++) {
    ...
    for (long long j = operand2_int; j < mx_pow(10, operand2_len); j++) {
      ...
      for (int p = operand2_len - j_len, j_ascii_i = 0; p < operand2_len;
           p++, j_ascii_i++) {
      ...
      for (long long k = result_int; k < mx_pow(10, result_len); k++) {
        ...
        for (int n = result_len - k_len, k_ascii_i = 0; n < result_len;
             n++, k_ascii_i++) {
...
bool mx_parse_number(char *value) {
  ...
  for (int i = 0; trimmed[i] != '\0'; i++) {
    if ((!mx_isdigit(trimmed[i]) && trimmed[i] != '?') &&
        !(trimmed[i] == '-' && i == 0)) {
      mx_strdel(&trimmed);
      return false;
    }
...
bool mx_parse_operation(char *value) {
  ...
  bool result = !(mx_strcmp(trimmed, "?") && mx_strcmp(trimmed, "+") &&
                  mx_strcmp(trimmed, "-") && mx_strcmp(trimmed, "*") &&
                  mx_strcmp(trimmed, "/"));
...
void mx_parse_arguments(char **operand1, char **operation, char **operand2,
                        char **result) {
